...
Where to drop `TAUT.md` for each of the 5 supported coding-agent CLIs.
...
The file is just the TAUT prompt by itself — no merge, no append.
...
The TAUT marker appears at column 1 of the file. These agents read their global instruction file at session start and apply it to every turn. No flags needed.
...
**IMPORTANT — model choice matters for cursor.** TAUT compliance on cursor depends on which underlying model is selected:

| Cursor model | Compression with TAUT v0.13.1 | Notes |
|---|---:|---|
...
| `gpt-5.3-codex` | **~78 % reduction** | Follows TAUT register cleanly; recommended |
| `gpt-5.2` | **~75 % reduction** | Similar to gpt-5.3-codex |

Recommended alias to make TAUT-compliant cursor invocations the default:

```bash
alias cursor-agent='agent --yolo --model gpt-5.3-codex'
```

Then `cursor-agent -p "your prompt"` will produce TAUT-compliant output.

## Verification command

After deploying, sanity-check that every file carries the TAUT prompt:

```bash
for p in ~/.claude/CLAUDE.md ~/.gemini/GEMINI.md \
         ~/.factory/AGENTS.md ~/.pi/agent/AGENTS.md ~/AGENTS.md; do
  grep -q "^# TAUT$" "$p" && echo "✓ $p" || echo "✗ $p"
done
```
